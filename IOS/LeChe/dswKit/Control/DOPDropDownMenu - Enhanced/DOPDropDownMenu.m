...
@property (nonatomic, strong) NSMutableArray *menuTitleList;   // 菜单文字
...
- (void)setTitleColumn:(NSInteger)column string:(NSString *)string {
    ((CATextLayer *)self.menuTitleList[column]).string = string;
}
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        [self.menuTitleList addObject:title];
        // indicator
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    self.menuTitleList = [NSMutableArray new];
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string {
    ...
    CGSize size = [astr boundingRectWithSize:CGSizeMake(280, 0) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil].size;
...
- (void)setDefaultTitleSingle:(DOPIndexPath *)indexPath {
    [self confiMenuWithSelectRow:indexPath.row];
}

- (void)setDefaultTitleDouble:(DOPIndexPath *)indexPath {
