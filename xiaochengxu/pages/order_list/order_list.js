...
Page({
  ...
  item_click:function(event){
    wx.navigateTo({
      url: '../order_detail/order_detail?id=' + event.currentTarget.id,
    })
  },
  ...
  getRefreshData() {
    ...
    params.page = page;
    network.requestLoading(getApp().globalData.API_URL+"/Home/Orders/ordersList", params, '正在加载数据', function (res) {
      index = res.data.list.length;
      sum_total = res.data.count;
