...
- (void)updatePageControl {
    ...
    if (_PageControlShowStyle == UIPageControlShowStyleLeft) {
        ...
        _pageControl.center = CGPointMake(kAdViewWidth / 2.0, kAdViewHeight - 18);
    } else {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_pageControl.currentPage < 2) {
        moveTimer.fireDate = [NSDate dateWithTimeInterval:5 sinceDate:[NSDate date]];
    } else {
        moveTimer.fireDate = [NSDate dateWithTimeInterval:3 sinceDate:[NSDate date]];
    }
