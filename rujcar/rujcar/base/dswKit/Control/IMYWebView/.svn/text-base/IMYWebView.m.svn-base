...
- (BOOL)callback_webViewShouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(NSInteger)navigationType {
    ...
    if (resultBOOL) {
        if (!tip) {
            tip = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
            tip.frame = CGRectMake(0, 0, 37.0f, 37.0f);
            tip.color = [UIColor lightGrayColor];
            tip.center = self.center;
            [self addSubview:tip];
        }
