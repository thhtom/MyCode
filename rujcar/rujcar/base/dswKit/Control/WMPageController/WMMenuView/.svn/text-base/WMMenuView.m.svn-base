...
@end
// 下划线的高度
static CGFloat const WMProgressHeight = 4.0;
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
    [self.progressView setProgress:index + 0.01];
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    NSArray *item = self.scrollView.allSubviews;
...
- (void)addItems {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        if (i != self.items.count - 1) {
            ...
            lineView.backgroundColor = [UIColor colorHex:@"cccccc"];
            lineView.left = item.right;
            lineView.width = 1;
            lineView.height = item.height - 20;
            lineView.top = 10;
